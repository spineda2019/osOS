...
pub inline fn panic(comptime source_info: FreeStandingSourceInfo) noreturn {
    ...
    const line_num: osformat.format.StringFromInt(
        @TypeOf(source_info.line),
        10,
    ) = comptime .init(source_info.line);

    terminal.write("Line: ");
    terminal.writeLine(comptime line_num.getStr());

    const column_num: osformat.format.StringFromInt(
        @TypeOf(source_info.column),
        10,
    ) = comptime .init(source_info.column);
